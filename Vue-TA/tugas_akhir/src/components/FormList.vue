<template>
  <div id="List">
    <div>
      <h1 class="Judul">{{ Judul }}</h1>
    </div>
    <div class="buton">
      <router-link to="/home">
        <button class="btn btn-dark mr-2">Back</button>
      </router-link>
      <button @click="TambahFom" v-show="c" class="btn btn-info">
        Tambah Anggota Keluarga
      </button>
    </div>
    <div id="tabel" v-show="ListData">
      <table class="table table-bordered mt-5 ml-5">
        <thead class="thead-dark mr-5">
          <tr>
            <th scope="col">No</th>
            <th scope="col">NIK</th>
            <th scope="col">Nama</th>
            <th scope="col">Jenis Kelamin</th>
            <th scope="col">Kepala Keluarga</th>
            <th scope="col">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in anggotaData" :key="item.id">
            <th scope="row">{{ index + 1 }}</th>
            <td>{{ item.nik }}</td>
            <td>{{ item.nama }}</td>
            <td>{{ item.jenis_kelamin }}</td>
            <td>{{ item.kepala_keluarga }}</td>
            <td>
              <router-link :to="{ name: `rere`, params: { id: item.id } }">
                <button class="btn btn-info w-auto m-1">
                  <img src="../assets/file.png" style="width: 20px" alt="" />
                </button>
              </router-link>
              <button
                type="button"
                @click="deleteAnggota(item.id, index)"
                class="btn btn-danger"
              >
                <img src="../assets/delete.png" style="width: 20px" alt="" />
              </button>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
    <div v-show="fom" class="fom">
      <form
        @submit.prevent="inputAnggota"
        class="border border-secondary p-4 mt-3"
      >
        <div class="row">
          <div class="col-6">
            <label for="">NIK</label>
            <input
              v-model="anggotaData.nik"
              type="text"
              class="form-control"
              placeholder="First name"
            />
          </div>
          <div class="col-6">
            <label for="">Tanggal Lahir</label>
            <input
              v-model="anggotaData.tanggal_lahir"
              type="date"
              class="form-control mb-3"
              placeholder="Last name"
            />
          </div>
        </div>
        <div class="row">
          <div class="col-6">
            <label for="">Nama</label>
            <input
              v-model="anggotaData.nama"
              type="text"
              class="form-control"
              placeholder="First name"
            />
          </div>
          <div class="col-6">
            <label for="">Agama</label>
            <input
              v-model="anggotaData.agama"
              type="text"
              class="form-control mb-3"
              placeholder="Last name"
            />
          </div>
        </div>
        <div class="row">
          <div class="col-6">
            <label for="">Jenis Kelamin</label>
            <select v-model="anggotaData.jenis_kelamin" class="custom-select">
              <option value="Pria">Pria</option>
              <option value="Wanita">Wanita</option>
            </select>
          </div>
          <div class="col-6">
            <label for="">Pedidikan</label>
            <input
              v-model="anggotaData.pendidikan"
              type="text"
              class="form-control mb-3"
              placeholder="Last name"
            />
          </div>
        </div>
        <div class="row">
          <div class="col-6">
            <label for="">Tempat Lahir</label>
            <input
              v-model="anggotaData.tempat_lahir"
              type="text"
              class="form-control"
              placeholder="First name"
            />
          </div>
          <div class="col-6">
            <label for="">Kepala Keluarga</label>
            <select v-model="anggotaData.kepala_keluarga" class="custom-select">
              <option value="Yes">Yes</option>
              <option value="No">No</option>
            </select>
          </div>
        </div>
        <button type="submit" class="btn btn-success mt-4">
          {{ Button }}
        </button>
        <button class="btn btn-danger mt-4 ml-2">Cancel</button>
      </form>
    </div>
  </div>
</template>

<script>
import anggotaService from "@/Services/anggotaService";
import Swal from "sweetalert2";
export default {
  name: "ListComp",

  data() {
    return {
      anggotaData: {
        agama: null,
        id_kk: null,
        jenis_kelamin: null,
        kepala_keluarga: null,
        nama: null,
        nik: null,
        pendidikan: null,
        tanggal_lahir: null,
        tempat_lahir: null,
      },
      ListData: true,
      fom: false,
      Judul: "Daftar Anggota Keluarga",
      c: true,
      Button: "Submit",
    };
  },

  methods: {
    getAnggota() {
      anggotaService
        .getAll()
        .then((response) => {
          this.anggotaData = response.data;
          console.log(this.anggotaData);
        })
        .catch((e) => {
          console.log(e);
        });
    },

    inputAnggota() {
      // inputAnggota
      let data = this.anggotaData;
      if (this.Button === "Submit") {
        anggotaService.create(data).then((response) => {
          console.log(response.data);
          alert("dada");
          //   Swal.fire({
          //     position: "center",
          //     icon: "success",
          //     title: "Your work has been saved",
          //     showConfirmButton: true,
          //     timer: 1500,
          //   });
          //   // .then(() => {
          //   //   this.$router.push("/ListAnggota");
          //   // });
          // })
          // .catch((e) => {
          //   console.log(e);
        });
      } else {
        // update anggota
        anggotaService.updateAnggota(data.id, data).then((response) => {
          console.log(response.data);
        });
        Swal.fire({
          position: "center",
          icon: "success",
          title: "Your work has been saved",
          // timer: 2000,
          showConfirmButton: true,
          timerProgressBar: true,
        });
        // .then(() => {
        //   this.$router.push("/home");
        // });
      }
    },
    //  Hapus Data Anggota
    deleteAnggota(id, index) {
      const swalWithBootstrapButtons = Swal.mixin({
        customClass: {
          confirmButton: "btn btn-success ",
          cancelButton: "btn btn-danger ",
        },
        buttonsStyling: true,
      });
      swalWithBootstrapButtons
        .fire({
          title: "Are you sure?",
          text: "You won't be able to revert this!",
          icon: "warning",
          showCancelButton: true,
          confirmButtonText: "Yes, delete it!",
          cancelButtonText: "No, cancel!",
          reverseButtons: true,
        })
        .then((result) => {
          if (result.isConfirmed) {
            anggotaService
              .deleteAnggota(id)
              .then((response) => {
                console.log(response.data);
                this.anggotaData.splice(index, 1);
              })
              .catch((e) => {
                console.log(e);
              });
            swalWithBootstrapButtons.fire(
              "Deleted!",
              "Your file has been deleted.",
              "success"
            );
          } else if (result.dismiss === Swal.DismissReason.cancel) {
            swalWithBootstrapButtons.fire(
              "Cancelled",
              "Your imaginary file is safe :)",
              "error"
            );
          }
        });
    },
    // tampil Data by id
    getDataAnggota(id) {
      anggotaService.getanggotaId(id).then((response) => {
        this.anggotaData = response.data;
        console.log(this.anggotaData);
      });
    },

    // Buat Ketentuan v-show
    TambahFom() {
      this.ListData = false;
      this.fom = true;
      this.Judul = "Tambah Anggota Keluarga";
      this.c = "false";
    },
  },

  mounted() {
    if (this.Judul == "Daftar Anggota Keluarga") {
      this.getAnggota();
    }
  },
};
</script>

<style scoped>
#List {
  margin-left: 250px;
  width: 85%;
}
.Judul {
  margin-left: 50px;
  margin-top: 1em;
  margin-bottom: 20px;
}
.buton {
  margin-left: 50px;
}
.fom {
  margin-left: 50px;
}
form {
  width: 85%;
}
#tabel {
  width: 92%;
}
</style>
